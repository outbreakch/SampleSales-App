...
export function RegisterClient({ initialLanguage }: { initialLanguage: StaffLocale }) {
    ...
    <main className="mx-auto flex min-h-screen max-w-6xl items-center px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="grid w-full gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        <Card className="bg-ink text-white">
          ...
          <h1 className="mt-4 font-serif text-4xl">{copy.registerHeroTitle}</h1>
          <p className="mt-4 max-w-md text-white/70">{copy.registerHeroDescription}</p>
          <div className="mt-8 rounded-[24px] border border-white/10 bg-white/5 p-5 text-sm text-white/75">
            {copy.localAccountAccess}
            <div className="mt-2 space-y-1 text-white">
              <p>{copy.localAccountRoleNote}</p>
              <p>{copy.localAccountAdminNote}</p>
            </div>
          </div>
        </Card>
        <Card className="bg-white/96">
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              <p className="text-xs uppercase tracking-[0.28em] text-stone">{copy.registerEyebrow}</p>
              <h2 className="mt-2 text-3xl font-semibold text-ink">{copy.registerTitle}</h2>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <Input placeholder={copy.firstNameLabel} value={firstName} onChange={(event) => setFirstName(event.target.value)} />
              <Input placeholder={copy.lastNameLabel} value={lastName} onChange={(event) => setLastName(event.target.value)} />
            </div>
