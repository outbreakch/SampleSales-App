...
export async function GET() {
  try {
    ...
    return apiServerErrorResponse(new Request("http://local/api/orders"), "orders.list.unhandled", error);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.data.customerEmail) {
      ...
      if (!delivery.ok) {
        if ("result" in delivery && delivery.result) {
          receiptResult = delivery.result;
        } else {
          receiptResult = { queued: false, provider: "unknown", note: delivery.error };
        }
        logger.error("receipt.send.checkout_failed", {
          ...requestLog,
          orderId: order.id,
          orderNumber: order.orderNumber,
    ...
    return apiServerErrorResponse(request, "orders.create.unhandled", error);
  }
}
